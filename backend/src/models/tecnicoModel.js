...
// const Tecnico = {
//   getAll: async () => {
//     const query = 'SELECT id, nombre, email, activo, es_admin FROM tecnicos ORDER BY nombre ASC;';
//     const { rows } = await pool.query(query);
//     return rows;
//   },
...
//     // Si enviaron una clave válida, actualizamos también el campo password
//     if (password && password.trim() !== '') {
//       const query = `
//         UPDATE tecnicos 
//         SET nombre = $1, email = $2, password = $3, activo = $4, es_admin = $5
...
//       return rows[0];
//     } else {
//       // Si el campo de contraseña viene vacío, no tocamos la contraseña existente
//       const query = `
//         UPDATE tecnicos 
//         SET nombre = $1, email = $2, activo = $3, es_admin = $4
//         WHERE id = $5
...
//   update: async (id, data) => {
//     const { nombre, email, activo, es_admin } = data;
//     const query = `
//       UPDATE tecnicos 
//       SET nombre = $1, email = $2, activo = $3, es_admin = $4
//       WHERE id = $5
...
//   getAll: async () => {
//     const query = 'SELECT * FROM tecnicos ORDER BY nombre ASC;';
//     const { rows } = await pool.query(query);
//     return rows;
//   },

//   // Crear un nuevo técnico (el estado 'activo' toma true por defecto en DB si no se pasa)
//   create: async (nombre, activo = true) => {
//     const query = `
//       INSERT INTO tecnicos (nombre, activo)
//       VALUES ($1, $2)
//       RETURNING *;
//     `;
//     const { rows } = await pool.query(query, [nombre, activo]);
//     return rows[0];
//   },
...
//     const query = 'DELETE FROM tecnicos WHERE id = $1 RETURNING *;';
//     const { rows } = await pool.query(query, [id]);
//     return rows[0]; // Retorna el registro eliminado para saber si existía
//   },

//   // Modificar un técnico existente
//   update: async (id, nombre, activo) => {
//     const query = `
//       UPDATE tecnicos 
...
//       RETURNING *;
//     `;
//     const { rows } = await pool.query(query, [nombre, activo, id]);
//     return rows[0];
//   },
// };
